...
### Added
- **CSV 資料匯入**: 系統設定頁面新增「CSV 資料匯入」功能 (支援 Drag & Drop)，可用於補入歷史交易紀錄與資產歷史趨勢。
- **連接嘉信 / 重新授權**: Settings 頁面新增一鍵跳轉按鈕，大幅優化 OAuth Token 過期或資料庫重置後的恢復流程。
- **認證狀態監控**: 新增 `/api/v1/auth/status` 端點，前端可即時顯示嘉信連線狀態（已連線/未連線）。
- **依賴更新**: 後端加入 `python-multipart` 支援，前端加入 `react-dropzone`。

### Fixed
- **風險指標修復**: 重構 Annual Return (總報酬年化法) 與 Max Drawdown (幾何收益率法)，徹底解決大額轉帳導致的指標偏差問題。
- **VaR 優化**: 改用參數法 (Parametric VaR) 提高科學性與準確度。
- **Settings UX**: 修復後端斷線時設定頁面欄位消失的問題，強化錯誤狀態顯示與欄位保護。
